...
def update_params(model, zs):
    ...
    if zs is not None:
        ...
        if "hidden_z" in zs:
            ...
            model.instance_embedding.data = \
                model.instance_embedding.data.mul(hidden_z)
            model.demultiplexer.demux_module.dense_1.weight.data = \
                model.demultiplexer.demux_module.dense_1.weight.data.mul(torch.cat((hidden_z, hidden_z), 0))
            for layer in range(num_layers):
...
def prune_model_with_z(zs, model):
    ...
    if "hidden_z" in zs:
        ...
        # Prune the instance embedding
        model.instance_embedding = torch.nn.parameter.Parameter(
            model.instance_embedding.index_select(1, index).clone().detach())
        ...
        index2 = torch.cat((index, torch.add(index, 768)), 0)
        model.demultiplexer.demux_module.demux_instance_embedding = \
            torch.nn.parameter.Parameter(
                model.demultiplexer.demux_module.demux_instance_embedding.index_select(1, index).clone().detach())
        model.demultiplexer.demux_module.dense_1 = \
            prune_linear_layer( model.demultiplexer.demux_module.dense_1, index2, dim=1)
