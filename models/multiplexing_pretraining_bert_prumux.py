...
from models.multiplexing_pretraining import (
    RetrievalHeadIndexPosDemultiplexingHierarchical,
    RetrievalHeadIndexDemultiplexing,
    MLPDemuxModule,
    IndexPosDemuxModule,
    IndexPosDemultiplexerTokenLevel,
    MLPDemultiplexerTokenLevel,
    RetrievalHeadMLPDemultiplexing,
...
from .modeling_bert import CoFiBertModel
...
class MuxedBertForMaskedLM(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if self.muxing_variant == "gaussian_hadamard":
            ...
            self.cross_instances_linear = torch.nn.Linear(config.hidden_size, d_model)
            self.cross_instances_layernorm = torch.nn.LayerNorm(d_model)
        elif (
            self.muxing_variant == "gaussian_attention_v2"
            or self.muxing_variant == "gaussian_attention_v2_no_context"
            or self.muxing_variant == "gaussian_attention_v2_cls_token"
        ):
            self.muxing_attention = ElectraLayer(config)
    ...
    def forward(
        ...
        if (
            ...
            embedding_output = torch.mean(embedding_output_attention, dim=1)
        elif self.muxing_variant == "gaussian_attention_v2_no_context":
            ...
            # embedding_output_attention_1 = embedding_output_attention_1[0]
            # embedding_output_attention_1 = embedding_output_attention_1 + embedding_output
            ...
            embedding_output = torch.mean(embedding_output_attention, dim=1)
        elif self.muxing_variant == "gaussian_attention_v2_cls_token":
            ...
            # apply attention layer across contextual embeddings in each position; add residual connection
            instance_cls_token = torch.ones(
                (
                    embedding_output_attention_2_input.shape[0],
                    1,
                ),
                device=input_ids.device,
                dtype=torch.long,
            )
            instance_cls_embedding = self.bert.embeddings(input_ids=instance_cls_token)
            embedding_output_attention_2_input = torch.cat(
                [instance_cls_embedding, embedding_output_attention_2_input], dim=1
            )
            embedding_output_attention_2 = self.muxing_attention_2(
                embedding_output_attention_2_input
            )
            embedding_output_attention_2 = embedding_output_attention_2[0]
            embedding_output_attention_2 = embedding_output_attention_2[:, 0:1, :]
            embedding_output_attention_2 = embedding_output_attention_2.view(
                -1, modified_seq_length, 1, embedding_dim
            )
            embedding_output_attention_2 = embedding_output_attention_2.permute(
                0, 2, 1, 3
            )
            embedding_output = embedding_output_attention_2.squeeze(1)
...
class MuxedBertForSequenceClassification(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if self.muxing_variant == "gaussian_hadamard":
            ...
            # for n, p in self.muxing_attention.named_parameters():
            #     # if "Layernorm" not in n and "bias" not in n:
            #     p.requires_grad = False
            # for n, p in self.muxing_attention_2.named_parameters():
            #     # if "Layernorm" not in n and "bias" not in n:
            #     p.requires_grad = False
        else:
            raise NotImplementedError()
    ...
    @classmethod
    def eval_from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], *model_args, **kwargs):
