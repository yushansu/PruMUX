...
from transformers import AutoTokenizer, EvalPrediction, GlueDataset, AutoModelForSequenceClassification, AutoConfig
...
def evaluate(model):
        ...
        for num_batch, inputs in enumerate(dataloader):
            ...
            with torch.no_grad():
                ...
                if i == 0:
                    ...
                    # total_examples += inputs['input_ids'].shape[0]
    ...
    metrics["examples/second"] = 1.0 * (t-1) * total_examples / total_infer_times
    return metrics
...
if __name__ == '__main__':
    ...
    muxN = int(sys.argv[2])
    msparsity = sys.argv[3]
    model_name_or_path = 'prumux-models/' + task_name_U + '/' + task_name_U + '_base'
    bs = 128*muxN
