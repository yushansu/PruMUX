...
def evaluate(model):
    ...
    for i in range(t):
        ...
        if label_ids is not None:
            ...
            lenid = len(final_label_ids)
            final_label_ids = final_label_ids[0:(lenid)]# - lenid%muxN)]
    ...
    metrics["examples/second"] = 1.0 * (t-1) * total_examples / total_infer_times # 1.0 / metrics["seconds/example"]
    return metrics
...
if __name__ == '__main__':
    ...
    task_name_U = sys.argv[1]
    msparsity = sys.argv[2]
    muxN = int(sys.argv[3])
    model_name_or_path = 'prumux-models/' + task_name_U + '/' + task_name_U + '_' + msparsity + '_' + str(muxN)
    model_name_or_path_best = model_name_or_path + "/best"
    ...
    model_class = MuxedBertForSequenceClassification

    zs = load_zs(model_name_or_path_best)


    # for compressed models
    if zs is None:
        model = model_class.eval_from_pretrained(model_name_or_path_best)
    # for full models with compression vectors zs
    else:
        # print("*****calling load_model*******")
        model = load_model(model_name_or_path_best, model_class, zs)
    ...
    print(f"Model path: {model_name_or_path_best}")
    for key in metrics:
